...
module.exports = {
        ...
        async.waterfall([
            ...
            function(countries, callback) {
                module.exports.createDocumentsGrid(function(err, docs) {
                    ...
                    cleanDocs = _.map(docs, function(doc) {
                      ...
                      if (cleanDoc.date_published) {
                          cleanDoc.state = 'available';
                      } else if (cleanDoc.expected_date_published &&
                                 moment(cleanDoc.expected_date_published) < moment()) {
                          cleanDoc.state = 'late';
                      } else {
                          cleanDoc.state = 'waiting';
                      }
            ...
            function(countries, docs, callback) {
              ...
              _.each(countries, function(country) {
                ...
                country.docs = _.map(docs, function(doc) {
                  delete doc.country;
                  return doc;
                });
              });
    ...
    createDocumentsGrid: function(callback) {
      documents.listAll(function(err, docs) {
        callback(err, docs);
      });
    },
