var _ = require('underscore'),
    documents = require('./documents'),
    moment = require('moment'),
    sites = require('./sites'),
    types;

types = [
    "Pre-Budget Statement",
    "Executive's Budget Proposal",
    "Enacted Budget",
    "Citizen's Budget",
    "In-Year Report",
    "Mid-year Review",
    "Year-End Report",
    "Audit Report"
];

module.exports = {
    getGrid: function(callback) {
        module.exports.createSiteGrid(function(err, grid) {
            if (err) {
                return callback(err);
            }

            documents.list({
                admin: true,
                approved: true
            }, function(err, docs) {
                if (err) {
                    return callback(err);
                }

                _.each(docs, function(doc) {
                    module.exports.updateGrid(grid, doc);
                });

                callback(null, grid);
            });
        });
    },
    updateGrid: function(grid, doc) {
        var cell,
            index,
            now = moment(),
            row;

        row = _.findWhere(grid, {
            country: doc.country
        });

        if (row) {
            index = _.indexOf(types, doc.type);

            cell = row.cells[index];

            if (!cell) {
                cell = row.cells[index] = {};
            }

            // no existing data
            if (!cell.year) {
                cell.year = doc.year;
                if (doc.available) {
                    cell.state = 'available';
                } else if (cell.expected_publication) {
                    if (moment(cell.expected_publication) < now) {
                        cell.state = 'late';
                    } else {
                        cell.state = 'waiting';
                    }
                }
            } else {
                if (cell.state === 'waiting' && doc.available) {
                    cell.state = 'available';
                    cell.year = doc.year;
                }
            }
        }
    },
    createSiteGrid: function(callback) {
        sites.listAll(function(err, sites) {
            var countries,
                grid;

            if (err) {
                return callback(err);
            }
            countries = module.exports.getSiteCountries(sites)
            grid = module.exports.arrangeGrid(countries);

            callback(null, grid);
        });
    },
    getSiteCountries: function(sites) {
        return _.groupBy(sites, function(site) {
            return site.country;
        });
    },
    arrangeGrid: function(countries) {
        var grid = [];

        _.each(countries, function(sites, country) {
            var country,
                existing,
                now = moment(),
                cells = new Array(types.length);

            existing = _.findWhere(grid, {
                country: country
            });

            if (existing) {
                cells = existing.cells;
            }

            _.each(sites, function(site) {
                debugger;
                var date = _.last(site.publication_dates),
                    index = _.indexOf(types, site.type);

                if (index >= 0) {
                    // could improve later; for now just last date found wins
                    cells[index] = {
                        state: moment(date) < now ? 'late' : 'waiting',
                        expected_publication: _.last(site.publication_dates)
                    }
                }
            });

            // only need to add if not already in grid
            if (!existing) {
                grid.push({
                    country: country,
                    cells: cells
                });
            }

        });
        return grid;
    }
};
