...
module.exports = {
    ...
    insert: function(options, callback) {
        ...
        userColl.update({
            _id: db.ObjectID(options.user_id)
        }, {
            $push: {
                sites: {
                    active: true,
     ...
     * @param {String} options.created_at ISOString value of the Date the site was added to the user.
     * @param {Function} callback
     */
    getSite: function(options, callback) {
        users.getUserById(options.user_id, function(err, user) {
            var site;

            if (err) {
                return callback(err);
            } else if (!user) {
                return callback("Could not find user with id " + options.user_id);
            }
