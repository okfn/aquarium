...
module.exports = {
    ...
    validateAndBuild: function(reqBody, assert) {
      ...
      var date_published = moment(reqBody.date_published, 'DD-MM-YYYY'),
          date_received = moment(reqBody.date_received, 'DD-MM-YYYY');
      if (date_published) {
        assert('date_published',
               'Date of publication should be in format DD-MM-YYYY')
          .isValidDate();
      }
      if (date_received) {
        assert('date_received',
               'Date received should be in format DD-MM-YYYY')
          .isValidDate();
      }
